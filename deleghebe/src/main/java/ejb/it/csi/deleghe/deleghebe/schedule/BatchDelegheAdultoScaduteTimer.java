...
@Stateless
public class BatchDelegheAdultoScaduteTimer {
	...
	//Singelton
		...
		public void batchGlobal() {
			...
			if(isServer1(hostName)) {
				...
				log.info(METHOD_NAME, "Esecuzione BatchDelegheAdultoScaduteTimer avviata");
				if (status > 0) {
					String msg = "Esecuzione scaduteAdultoDelegheBatch... ";
					status = batchBean.scaduteAdultoDelegheBatch(gCal);
					if (status > 0) msg += "completata!";
					log.info(METHOD_NAME, msg);
				}
		
				log.info(METHOD_NAME, "Esecuzione BatchDelegheAdultoScaduteTimer " + ((status > 0)? "completata" : "annullata"));
			} else {
