...
@Stateless
public class BatchDelegheInScadenzaTimer {
	//Singelton
	@Inject
	private LogUtil log;
	...
	@Schedule(second = "0", minute = "0", hour = "5", persistent = false)
	...
	public void batchGlobal() {
		...
		if(isServer1(hostName)) {
			...
			log.info(METHOD_NAME, "Esecuzione BatchDelegheInScadenzaTimer avviata");
			if (status > 0) {
				String msg = "Esecuzione inScadenzaDelegheBatch... ";
				if (status > 0) msg += "completata!";
				log.info(METHOD_NAME, msg);
			}
	
			log.info(METHOD_NAME, "Esecuzione BatchDelegheInScadenzaTimer " + ((status > 0)? "completata" : "annullata"));
		} else {
