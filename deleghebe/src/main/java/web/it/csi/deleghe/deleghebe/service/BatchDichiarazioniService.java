...
public class BatchDichiarazioniService extends BaseService<BatchDeleghe, BatchDelegheResponse> {
	...
	protected BatchDelegheResponse execute(BatchDeleghe req) {
		...
		try {
			int counter = batchBean.scadenzaDichiarazioni();
			res.setCntScadute(counter);
